globals [ max-sheep max-wolves max-grasses walk escape]  ; don't let the sheep population grow too large
...
patches-own [ countdown fertility ]    ; this is for the sheep-wolves-grass model version
...
to go
  ...
  if not any? sheep and count wolves > max-wolves [ user-message "The wolves have inherited the world"
    stop ]
  if not any? sheep and not any? wolves [ user-message "This is really a green planet!"
  stop ]
  ...
  ask sheep [
    ...
    set energy energy - loss-step
    death
    ifelse energy > reproduce-threshold [
      find-sheep
      reproduce-sheep
    ] [ move ]
...
;      reproduce-wolves
    ...
    ifelse any? sheep in-radius sight-range [
      find-wolves
    ] [ fd 0.5 move ]
      reproduce-wolves
  ]
...
to hunt-grasses
  let target-grass one-of grasses in-radius sight-range
...
to hunt-sheep
  let target-sheep one-of sheep in-radius sight-range
  ifelse target-sheep != nobody [
    face target-sheep
    ask target-sheep [ flee ]
    fd walk
...
to grow-grass
  let current-fertility [fertility] of patch-here
  let new-energy (energy + grass-gain-from-fertility / 100 * current-fertility)
...
to eat-sheep
  ...
  if a-sheep-here != nobody [
    ...
    ifelse new-energy > 100 [set energy 100][set energy new-energy]
    ask neighbors [ set fertility fertility + (1 - wolf-gain-from-food / 900) * [energy] of a-sheep-here]
    ask patch-here [ set fertility fertility + (1 - wolf-gain-from-food / 900) * [energy] of a-sheep-here]
...
to find-wolves
  let target-wolf one-of wolves in-radius sight-range
  ifelse target-wolf != nobody [
    face target-wolf
    fd walk
  ][ move ]
end
...
"grass / 4" 1.0 0 -10899396 true "" "plot count grasses"
