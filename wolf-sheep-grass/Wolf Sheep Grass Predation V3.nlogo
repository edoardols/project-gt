globals [ max-sheep max-wolves max-grasses walk escape group-range]  ; don't let the sheep population grow too large
...
turtles-own [ energy target-x target-y]       ; both wolves and sheep have energy
...
to setup
  ...
  ask patches [
    set pcolor green + 2
    set countdown random grass-regrowth-time * 2
  ]

  create-grasses initial-number-grass  ; create the sheep, then initialize their variables
  [
    set shape  "plant"
    set color green - 2
...
to go
  ...
  if not any? turtles [ user-message "The eco-system did not survive" stop ]
  ; stop the model if there are no wolves and the number of sheep gets very large
  if not any? wolves and count sheep > max-sheep [ user-message "The sheep have inherited the world"
    stop ]
  if not any? sheep and count wolves > max-wolves [ user-message "The wolves have inherited the world"
    stop ]
  ; if not any? sheep and not any? wolves [ user-message "This is really a green planet!" stop ]
  if count sheep = 1 and count wolves = 0 and count grasses > 0 [ user-message "The damn sheep!"
    stop ]

  ask grasses [
    set energy energy - loss-step / 10
    death
    ; reproduce-grasses
  ]
  ...
  ask wolves [
    ...
    group-wolves
    set energy energy - loss-step
    if any? sheep in-radius sight-range [move]
    death
    reproduce-wolves
  ]
...
to  hunt-grasses
  let target-grass min-one-of grasses in-radius sight-range [distance myself]
  ; let nearby-grasses grasses in-radius sight-range
  ; let target-grass max-one-of nearby-grasses [energy]
  ifelse target-grass != nobody [
    face target-grass
    fd walk
  ][ move ]
end
...
to hunt-sheep
  let target-sheep min-one-of sheep in-radius (sight-range + 2) [distance myself]
  ; let nearby-sheep sheep in-radius sight-range
  ; let target-sheep max-one-of nearby-sheep [energy]
...
to flee
  ...
  rt 180  ;; Turn around
  lt random 45  ;; Randomly adjust direction
  rt random 45
  fd escape ;; Move forward to escape
  set energy energy - 0.5 ]
end

to group-sheep
  let nearby-sheep other sheep in-radius group-range
  if any? nearby-sheep [
    let avg-x mean [xcor] of nearby-sheep
    let avg-y mean [ycor] of nearby-sheep
    set target-x avg-x
    set target-y avg-y
  ]
  move-towards-target
...
to group-wolves
  let nearby-wolves other wolves in-radius group-range
...
to eat-grass
  let grass-here one-of grasses-here
  if grass-here != nobody [
    let new-energy (energy + sheep-gain-from-food / 100 * [energy] of grass-here)
    ifelse new-energy > 100 [set energy 100][set energy new-energy]
    ask grass-here [ die ]
  ]
end

to eat-sheep
  let a-sheep-here one-of sheep-here
  if a-sheep-here != nobody [
    let new-energy (energy + wolf-gain-from-food / 100 * [energy] of a-sheep-here)
    ifelse new-energy > 100 [set energy 100][set energy new-energy]
    ask a-sheep-here [ die ]
    fertilization
  ]
end

to fertilization
  ;; Generare una o due nuove erbe solo se non c'è già erba nella stessa patch
     let num-new-grass random 9 + 1
     repeat num-new-grass [
      let nearby-patch one-of patches in-radius 1 with [count grasses-here = 0]
      if nearby-patch != nobody [ ask nearby-patch [
 ;; TODO make it work
     hatch 1 [
           set shape "plant"
           set color green - 2
...
to reproduce-grasses
  ifelse [countdown] of patch-here <= 0 [
   if random-float 100 < grass-reproduce [
     ask patches in-radius 1 with [not any? grasses-here with [self != myself]] [
       sprout-grasses 1 [
         set shape "plant"
         set color green - 2
         set size 1
  ...
  ][ask patch-here [set countdown countdown - 1]]
end

to reproduce-sheep  ; sheep procedure
  let my-partner one-of sheep-here with [self != myself]
  if my-partner != nobody [
    if random-float 100 < sheep-reproduce [  ; throw "dice" to see if you will reproduce
      hatch 1 [ rt random-float 360 fd 1 ] ;set energy ([energy] of myself + [energy] of my-partner) / 4 ]
...
to reproduce-wolves  ; wolf procedure
  let my-partner one-of wolves-here with [self != myself]
  if my-partner != nobody [
...
to display-turtles-labels
  ...
  if show-turtles-energy? [
    ...
    ask grasses [ set label round energy ]
  ]

  if show-wolves-energy [ ask wolves [ set label round energy ] ]
  if show-sheep-energy [ ask sheep [ set label round energy ] ]
  if show-grasses-energy [ ask grasses [ set label round energy ] ]
end
...
"grass" 1.0 0 -10899396 true "" "plot count grasses"
...
setup</setup>
  ...
  <experiment name="coexistance" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>count sheep</metric>
    <metric>count grasses</metric>
    <enumeratedValueSet variable="wolf-gain-from-food">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wolf-reproduce">
    ...
    <enumeratedValueSet variable="initial-number-grass">
      <value value="700"/>
    </enumeratedValueSet>
    <steppedValueSet variable="initial-number-sheep" first="100" step="10" last="300"/>
    <steppedValueSet variable="hunger-threshold" first="20" step="10" last="50"/>
    <steppedValueSet variable="sheep-gain-from-food" first="0" step="10" last="100"/>
    <steppedValueSet variable="grass-regrowth-time" first="20" step="5" last="50"/>
    <steppedValueSet variable="reproduce-threshold" first="0" step="10" last="50"/>
    <enumeratedValueSet variable="show-sheep-energy">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="sight-range">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="show-wolves-energy">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="grass-reproduce">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="grass-gain-from-fertility">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="show-grasses-energy">
      <value value="false"/>
    </enumeratedValueSet>
