...
to setup
  ...
  ; create-patches initial-number-grasses ; create the grasses, then initialize their variables
...
to go
  ...
  ask patches [
    ; r1 = regrowth
    if ( pcolor = brown ) and ( random-float 1 < (abs r1) ) [set pcolor green]
    ; a11 = self limitation
    ; a12 = predation mechanism from sheep over grasses
    if ( pcolor = green ) and ( any? sheep-here ) and ( random-float 1 < (abs a12) ) [set pcolor brown]
  ...
  ask sheep [
    ...
    let isgreen? false
    ask patch-here [ if pcolor = green [set isgreen? True]]
    ;if ( random-float 1 < (abs a21) ) and ( is-patch-set? pcolor = green ) [ hatch 1 [ rt random-float 360 fd 1 ] ]
    if ( random-float 1 < (abs a21) ) and ( isgreen? ) [ hatch 1 [ rt random-float 360 fd 1 ] ]
  ...
  ask wolves [
    ...
    if ( random-float 1 < (abs a32) ) and ( any? sheep-here ) [ hatch 1 [ rt random-float 360 fd 1 ] ]
